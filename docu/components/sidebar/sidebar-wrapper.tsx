...
export default function SidebarWrapper({links, isAdmin}: SidebarLinkInfo & {isAdmin: boolean | null}){
    ...
    if (isMobile) {
        return(
            <div className={clsx(
                "absolute top-0 z-60 flex flex-col gap-4 grow bg-background styled-scrollbar border-r border-border h-full max-h-[calc(100svh_-_65px)] p-5 w-68",
                {"animate-slide-out": !isOpenMobile},
                {"animate-slide-in": isOpenMobile}
            )}
    ...
    return(
        <div className={clsx(
            "hidden md:flex flex-col gap-4 grow bg-background styled-scrollbar border-r border-border h-full max-h-[calc(100svh_-_65px)] p-5",
            {"w-auto": state === "closed"},
            {"w-68": state === "expanded"}
            )}
